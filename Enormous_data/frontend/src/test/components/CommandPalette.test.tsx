...
describe('CommandPalette', () => {
  it('opens, filters destinations, and keeps navigation searchable', async () => {
    ...
    await user.type(screen.getByPlaceholderText('搜索页面、指标或工作流'), '用户');

    expect(await screen.findByText('用户分层')).toBeInTheDocument();
  ...
  it('supports keyboard search and enter navigation', async () => {
    ...
    const input = screen.getByPlaceholderText('搜索页面、指标或工作流');
