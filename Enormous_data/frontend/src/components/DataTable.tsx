...
export function DataTable({ data, isLoading = false, onInspectRow, onSort, sortDirection = 'asc', sortKey = 'event_time' }: DataTableProps) {
      ...
      <div className="table-scroll" aria-label="行为明细滚动区域">
        <table aria-label="行为明细" className="data-table">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={8} className="empty-cell">正在加载行为记录</td>
              </tr>
            ) : null}
            {!isLoading && sortedRows.length === 0 && (
              <tr>
                <td colSpan={8} className="empty-cell">暂无匹配的行为记录</td>
              </tr>
            )}
