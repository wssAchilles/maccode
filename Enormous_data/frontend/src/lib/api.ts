...
export const api = {
  ...
  dashboardSlice: (params: DashboardSliceFilters) => request<DashboardSlice>(withQuery('/dashboard/slice', params)),
  controlledQuery: (query: string) =>
    request<ControlledQueryResult>('/query/controlled', {
      method: 'POST',
  ...
  recommendationCandidates: (params: { source?: string; limit?: number } = {}) =>
    request<RecommendationCandidate[]>(withQuery('/recommendations/candidates', params)),
  recommendationQuality: () => request<RecommendationQuality>('/recommendations/quality'),
  recommendationEvaluation: () => request<RecommendationEvaluation>('/recommendations/evaluation'),
  ...
  anomalyIncidents: (limit = 50) => request<AnomalyIncident[]>(withQuery('/anomalies/incidents', { limit })),
  anomalyRootCause: (params: { incident_id?: string } = {}) =>
    request<AnomalyRootCause[]>(withQuery('/anomalies/root-cause', params)),
  anomalyEvaluation: () => request<AnomalyEvaluation>('/anomalies/evaluation'),
  ...
  experimentResults: (params: { experiment_key?: string } = {}) =>
    request<ExperimentResult[]>(withQuery('/experiments/results', params)),
  experimentUplift: (params: { experiment_key?: string } = {}) =>
    request<ExperimentUplift>(withQuery('/experiments/uplift', params)),
  ...
  featureMartFeatures: () => request<FeatureMartFeature[]>('/feature-mart/features'),
  featureMartReadiness: () => request<FeatureMartReadiness>('/feature-mart/readiness'),
  ...
  forecastingEvaluation: () => request<ForecastingEvaluation>('/forecasting/evaluation'),
  ...
  affinityCentrality: (params: { community_id?: string; limit?: number } = {}) =>
    request<AffinityCentrality[]>(withQuery('/affinity/centrality', params)),
  ...
  optimizationImpact: () => request<OptimizationImpact>('/ops/optimization-impact'),
  liveWeatherCurrent: () => request<LiveWeatherCurrent>('/live-weather/current'),
  liveWeatherForecast: () => request<LiveWeatherForecast>('/live-weather/forecast'),
  liveWeatherSummary: () => request<LiveWeatherSummary>('/live-weather/summary'),
  liveTrainingStatus: () => request<LiveTrainingStatus>('/live-training/status'),
  liveTrainingRefresh: () => request<{ status: string; run_id: string }>('/live-training/refresh', { method: 'POST' }),
  liveTrainingMetrics: () => request<LiveTrainingMetrics>('/live-training/metrics'),
  liveTrainingImpact: () => request<LiveWeatherImpact>('/live-training/impact'),
  liveTrainingForecastImpact: () => request<LiveWeatherForecastImpact>('/live-training/forecast-impact'),
  refresh: () => request<{ status: string; job_id: string }>('/refresh', { method: 'POST' }),
  table: (params: { page: number; size: number; event_type?: string; category_level1?: string; brand?: string }) =>
