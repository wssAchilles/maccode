...
本系统采用经典的 B/S (Browser/Server) 架构模式，通过分层设计实现关注点分离，确保系统的可扩展性、可维护性和高可用性。图~\ref{fig:system_arch}展示了储能智能管理系统的总体架构。
...
\begin{itemize}
    ...
    \item \textbf{定时任务调度}：提供 GAE Cron 触发的 \texttt{/tasks/fetch-data} 与 \texttt{/tasks/train-model} 路由，内部调用调度器抓取外部数据、启动模型训练（Cron Header 校验保障安全）；本地开发可跳过 Header 检查。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cloud Storage}：对象存储服务，用于托管训练数据集（CSV 文件）和机器学习模型文件，并持久化模型元数据（\texttt{model\_metadata.json}）；
...
移动端应用采用Material Design 3设计语言和卡片式布局，数据可视化集成\texttt{fl\_chart}库实现高性能图表渲染。登录后进入\textbf{三栏主导航（底部 Tab）}:\texttt{能源优化}、\texttt{数据分析}、\texttt{历史记录}。其中，能源优化仪表盘提供工业级沙盒参数（默认 5,000 kWh / 2,000 kW，可调初始 SOC、温度 What-if），调接口 \texttt{/api/optimization/run} 返回策略、节省额、特征重要性；数据分析页支持 CSV 上传+质量仪表盘（描述统计、缺失值、相关矩阵）；历史页调用 \texttt{/api/history/list} / \texttt{/api/history/delete} 管理记录。核心界面包括用户登录、系统主页（SOC 监控）、负载预测曲线、充放电调度计划、质量仪表盘和历史数据分析（详见图4.x系列截图）。
...
图~\ref{fig:app_auth_home}展示了应用的入口界面。图~\ref{fig:app_auth_home}(a) 为基于 Firebase Authentication 实现的安全登录页面，支持邮箱/密码认证方式，登录成功后 Firebase 返回的 ID Token 将作为后续 API 请求的身份凭证。图~\ref{fig:app_auth_home}(b) 是用户登录后进入的系统主页 (Dashboard)，采用信息密度适中的卡片布局，用户可一目了然地查看当前电池荷电状态 (SOC)、实时电价以及今日能耗概览，实现对储能系统的全局感知。
...
\subsubsection{状态管理与数据交互}

应用采用\textbf{Provider}模式实现全局状态管理，定义\texttt{AuthProvider}（认证）、\texttt{DataProvider}（实时数据）、\texttt{OptimizationProvider}（优化结果）三类状态对象。

网络请求封装于\texttt{ApiService}类，使用Dart的\texttt{http}包发起异步HTTP请求。每次请求携带Firebase ID Token进行身份认证，实现完整的加载状态管理和错误处理机制（SnackBar提示），平均延迟控制在300ms以内（详见第5章性能评估）。
...
后端服务采用Docker容器化封装,Dockerfile采用\texttt{python:3.11-slim}基础镜像,分层安装依赖（\texttt{gurobipy, scikit-learn, flask}等），暴露8080端口，使用Gunicorn作为WSGI服务器。

容器化优势：环境一致性、依赖隔离、快速部署、可移植性（可部署至任何支持Docker的云平台）。

\subsection{Google App Engine 部署}

后端部署于GAE Flexible Environment，通过\texttt{app.yaml}配置运行时（Python 3.11）、实例规格（F4高性能实例）、自动扩缩容策略（\texttt{min\_instances: 0}节省成本，\texttt{max\_instances: 1}控制账单）及环境变量（API密钥、Gurobi许可证等）。

\textbf{核心优势}：
\begin{itemize}
    \item 自动扩缩容：无流量时关闭实例，峰值时弹性扩容
    \item 零停机部署：新版本健康检查通过后自动切换流量
    \item 按需计费：免费配额（每日28实例小时）足够覆盖日常使用
\end{itemize}

部署命令：\texttt{gcloud builds submit} + \texttt{gcloud app deploy}。
...
本章详细介绍了储能智能管理系统的架构设计与工程实现。从系统总体架构出发，阐述了 B/S 分层架构的设计理念；在后端服务开发中，展示了 Flask RESTful API、Firebase 集成和自动化任务调度的实现细节；在移动端应用开发中，介绍了 Material Design 3 设计语言、核心功能模块和 Provider 状态管理架构；最后，通过 Docker 容器化和 Google App Engine 部署，证明了系统具备生产环境的运行能力。
