...
\newcommand{\ReportTitleA}{家庭储能优化调度系统}
\newcommand{\ReportTitleB}{——基于机器学习与数学规划的设计与实现} % 如果不需要第二行可以留空
...
为了直观展示上述时序特征的构造过程，图~\ref{fig:feature_engineering_process} 描绘了如何从单一负载序列中提取滞后与统计特征的原理。
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    % Draw Time Axis
    \draw[timeline] (0,0) -- (12,0) node[right] {时间 $t$};

    % Draw Signal
    \draw[blue, thick, smooth] plot coordinates {(0,1) (1,1.2) (2,1.5) (3,2.0) (4,1.8) (5,2.2) (6,1.0) (7,1.1) (8,1.3) (9,1.6) (10,2.5) (11,1.5)};

    % Current Time Feature t
    \draw[dashed, red] (10,0) -- (10,2.5);
    \node[below, red] at (10,0) {$t$};
    % Move red text right to avoid clash with green window label
    \node[right, red] at (10.1, 2.5) {$y_t$ (目标)};

    % Lag Feature t-1
    \draw[dotted] (9,0) -- (9,1.6);
    \node[below] at (9,0) {$t-1$};
    \draw[->, orange, thick] (9,1.6) -- (9.8, 1.6) node[midway, above] {\scriptsize Lag 1};

    % Lag Feature t-3
    \draw[dotted] (7,0) -- (7,1.1);
    \node[below] at (7,0) {$t-3$};
    \draw[->, orange, thick] (7,1.1) to[bend left=20] (9.8, 1.3);
    \node[orange] at (8.5, 1.4) {\scriptsize Lag 3};

    % Rolling Window Block
    \fill[green!10] (6,0) rectangle (10,2.5);
    \draw[green!60!black, dashed] (6,0) -- (6,2.5);
    \node[below] at (6,0) {$t-w$};
    \draw[<->, green!60!black] (6,2.8) -- (10,2.8) node[midway, above] {Rolling Window $w$};

    % Aggregation Arrow
    % Move stats box further right and up
    \node[draw, rectangle, fill=white, drop shadow] (agg) at (12.8, 3.2) {$\mu = \frac{1}{w}\sum y_i$};
    \draw[->, green!60!black, thick] (8, 2.8) .. controls (9, 3.2) .. (agg);

    % Feature Vector Construction
    \node[draw, rectangle, rounded corners, fill=yellow!10, minimum width=4cm, minimum height=1cm, align=left] (vector) at (5, -2) {
    \textbf{特征向量} $\mathbf{x}_t$: \\
    $[ \text{Hour}_t, \text{Temp}_t, \color{orange}y_{t-1}, y_{t-3}, \dots, \color{green!60!black}\mu_w, \sigma_w \color{black}]$
    };

    \draw[->, dashed] (9,0) -- (vector);
    \draw[->, dashed] (agg) -- (vector);

  \end{tikzpicture}
  \caption{时序特征构造原理示意图：展示了如何基于当前时刻 $t$ 回溯历史数据，提取滞后特征（Lag）与滑动窗口统计特征（Rolling Stats）。}
...
\subsection{数据治理与工程化实践}

为了确保实验结果的可靠性与可追溯性（Reproducibility），本项目建立了严格的数据管理规范。我们采用“数据快照”策略，将原始数据、预处理脚本与生成的特征数据集进行关联管理。任何一次模型训练都会记录对应的数据集状态与预处理配置，确保在不同时间点均能复现相同的实验环境。

\subsection{核心代码实现}
为了确保特征工程的可复现性，我们将上述逻辑封装于 \texttt{FeatureEngineer} 类中，并严格遵循“无状态变换”原则，即在测试集上复用训练集计算得到的统计量（如均值、方差），严禁重新计算以防止信息泄露。这种严谨的数据治理流程有效防止了未来信息泄露和实验结果的不可复现。
...
\begin{itemize}
  ...
  \item \texttt{max\_depth} (最大树深): [10, 20, None]。限制树深是防止过拟合的最有效手段。
  \item \texttt{min\_samples\_split} (分裂最小样本数): [2, 5, 10]。较大的值能抑制过细的划分。
\end{itemize}

\subsubsection{调优结果}
经过 5-Fold 交叉验证，通过最小化 RMSE 指标，我们确定了最优参数组合：$\{ \texttt{n\_estimators}: 100, \texttt{max\_depth}: \text{None}, \texttt{min\_samples\_split}: 2 \}$。该组合下的模型在验证集上的 MSE 较默认参数降低了 15\%。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    \textbf{随机森林 (Random Forest)} & \textbf{1.338}     & \textbf{0.985}    & \textbf{0.92}      \\
...
\begin{figure}[htbp]
  \centering
  \fbox{\includegraphics[width=0.8\textwidth]{assets/prediction_vs_actual.pdf}}
  \caption{模型对典型日负载的预测曲线对比}
...
保障电池的长期健康运行是系统设计的关键考量。虽然更复杂的电化学寿命模型（如雨流计数法）可以提供精确的老化估算，但考虑到计算复杂度和实时性要求，本系统当前阶段采用了更具鲁棒性的\textbf{基于物理约束的保护策略}。

具体而言，我们在优化模型中引入了严格的硬约束来规避电池运行的高风险区间：
\begin{enumerate}
  \item \textbf{最大放电深度 (DOD) 限制}：严格限制 $SOC \ge 10\%$，防止电池因过度放电导致电压塌陷和不可逆的化学损伤；
  \item \textbf{充放电倍率 (C-rate) 限制}：限制最大充放电功率不超过 $1.0C$（即1小时充满的速率），避免大电流产生的焦耳热积累加速电池老化；
  \item \textbf{容量防过充}：设定 $SOC \le 90\%$ 的上界，留出安全裕度。
\end{enumerate}

实验表明，这种基于规则的约束机制在保证经济效益的同时，有效地将电池维持在健康的运行窗口内，是一种兼顾工程可行性与设备安全的最佳实践。
...
本项目为团队协作完成，两位成员在项目中承担了不同的核心职责，通过紧密配合实现了端到端的系统闭环。具体分工如下：

\subsection{许子祺（23030327）—— 算法建模与后端核心}
主要负责项目的\textbf{数据科学内核}与\textbf{后端服务逻辑}，具体工作包括：
\begin{itemize}
  \item \textbf{核心算法实现}：基于 Gurobi 实现混合整数规划 (MIP) 调度模型，构建目标函数与物理约束；实现随机森林 (Random Forest) 负载预测算法及 SHAP 模型解释模块。
  \item \textbf{数据工程 (ETL)}：设计并实现 \texttt{EnergyDataProcessor} 类，完成多源异构数据的清洗、聚合、重采样及特征工程（Lag/Rolling 特征构建）。
  \item \textbf{后端 API 开发}：基于 Flask 框架搭建 RESTful API 服务，实现数据流转、认证鉴权及与 Google Cloud 服务的对接。
  \item \textbf{数学建模}：负责优化问题的形式化描述，将现实业务场景抽象为数学规划问题。
\end{itemize}

\subsection{莫文涛（23030315）—— 全栈应用与云端架构}
主要负责项目的\textbf{用户交互体验}与\textbf{系统基础设施}，具体工作包括：
\begin{itemize}
  \item \textbf{前端应用开发}：基于 Flutter 框架开发跨平台客户端，实现响应式布局、状态管理及复杂图表（fl\_chart）的高性能渲染。
  \item \textbf{UI/UX 设计}：设计包括实时监控仪表盘、数据分析大屏及优化策略展示在内的可视化界面，提升用户体验。
  \item \textbf{云原生架构部署}：负责 Docker 容器化封装，搭建 Google App Engine (GAE) 无服务器架构，配置 Cloud Scheduler 定时任务及 Firestore 数据库。
  \item \textbf{系统测试与集成}：编写测试用例，负责前后端联调及系统的全链路集成测试。
\end{itemize}
...
\begin{codeblock}[language=Python]{机器学习训练流程 (ml\_service.py)}
  ...
  print(f"   [OK] 数据读取成功: {len(df)} 行 × {len(df.columns)} 列")
  ...
  print(f"   [OK] Temperature 缺失值已用均值填充: {mean_temp:.2f}°C")
  else:
  print(f"   [OK] 无缺失值")
  ...
  # 划分训练集和测试集
  print(f"\\n[SPLIT] 划分数据集 (训练集: {int((1-test_size)*100)}%, 测试集: {int(test_size*100)}%)...")
