...
class TrackletReassociationSummary:
    ...
    relinked_count: int
    rejected_count: int
    model_reference: str = "geometry_only_tracklet_reassociation"
...
class TrackletReAssociationService:
    """Offline short-gap tracklet linker using BEV kinematics and class consistency."""
    ...
    def relink_reports(
        ...
        for child_id, child in endpoints.items():
            best: TrackletAssociation | None = None
        ...
        if not associations:
            return reports, TrackletReassociationSummary(candidates, 0, candidates)
    ...
    def _score(
        ...
        score = (
            0.55 * max(0.0, 1.0 - error / self.max_position_error_m)
            + 0.25 * direction_score
            + 0.20 * max(0.0, 1.0 - speed_delta / 25.0)
        )
    ...
    def _track_endpoints(reports: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
        ...
        for report_index, report in enumerate(reports):
            ...
            for track in report.get("active_tracks", []):
                ...
                if track.get("ground_x_m") is None or track.get("ground_y_m") is None:
                    continue
                item = dict(track)
                item["_report_index"] = report_index
        ...
        for tracker_id, tracks in grouped.items():
            ordered = sorted(tracks, key=lambda item: int(item["_report_index"]))
            if len(ordered) < 2:
                continue
            first = ordered[0]
            last = ordered[-1]
            endpoints[tracker_id] = {
                ...
                "first_frame": int(first["_report_index"]),
                "last_frame": int(last["_report_index"]),
